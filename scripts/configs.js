...
const getConfigForEnvironment = async (environment) => {
  ...
  try {
    ...
    configJSON[':expiry'] = Math.round(Date.now() / 1000) + 3600;
 ...
 */
const getConfigValue = async (configParam, environment) => {
  const env = environment || calcEnvironment();
...
*/
const getHeaders = async (scope, environment) => {
  const env = environment || calcEnvironment();
  const config = await getConfigForEnvironment(env);
...
export const getAemContentPath = () => {
  ...
  if (window.hlx && window.hlx.codeBasePath) {
    [authorContentPath] = window.hlx.codeBasePath.match(/^[^.]+/);
...
const checkIsAuthenticated = () => !!getCookie('auth_dropin_user_token') ?? false;
